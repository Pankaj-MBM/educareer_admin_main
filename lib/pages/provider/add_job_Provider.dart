...
class AddJobProvider extends StatefulWidget {
  @override
  _AddJobProviderState createState() => _AddJobProviderState();
}

class _AddJobProviderState extends State<AddJobProvider> {
  ...
  Future<void> _addJobProvider() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      DocumentReference docRef = await FirebaseFirestore.instance.collection('job_providers').add({
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Job provider added successfully')),
      );
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 20),
                    ElevatedButton(
                      onPressed: _addJobProvider,
                      child: Text('Add Job Provider'),
                    ),
                  ],
                ),
              ),
